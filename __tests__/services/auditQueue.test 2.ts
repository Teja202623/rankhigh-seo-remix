...
describe('Audit Queue Service', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Reset mocks
    (prisma.audit.findFirst as jest.Mock).mockClear();
    (prisma.audit.update as jest.Mock).mockClear();
  });
  ...
  describe('canRunAudit', () => {
    ...
    it('should prevent audit if last one completed less than 1 hour ago (FREE tier limit)', async () => {
      const now = new Date();
      const 50MinutesAgo = new Date(now.getTime() - 50 * 60 * 1000);
    ...
    it('should calculate correct nextAllowedTime', async () => {
      const now = new Date();
      const 55MinutesAgo = new Date(now.getTime() - 55 * 60 * 1000);
  ...
  describe('Audit Queue Integration', () => {
    ...
    it('should track audit cooldown across multiple checks', async () => {
      ...
      const now = new Date();
      const 30MinutesAgo = new Date(now.getTime() - 30 * 60 * 1000);

      const recentAudit = {
        id: 'audit-recent',
        status: 'COMPLETED',
        storeId,
        createdAt: 30MinutesAgo,
      };
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very recent audit (seconds ago)', async () => {
      const now = new Date();
      const 5SecondsAgo = new Date(now.getTime() - 5 * 1000);
